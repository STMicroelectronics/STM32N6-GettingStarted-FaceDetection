# __Face Detection Getting Started__

---

This project provides a real-time embedded environment for STM32N6 microcontroller to execute [STEdgeAI](https://www.st.com/en/development-tools/stedgeai-core.html) generated models, specifically targeting the face detection application. The code prioritizes clarity and understandability over performance, making it an ideal starting point for further development.

![Image sample](_htmresc/sample.PNG)
Box and detected keypoints are displayed.

This is a standalone project that can be deployed directly to hardware. It is also integrated into the [ST ModelZoo repository](https://github.com/STMicroelectronics/stm32ai-modelzoo-services), and is required to deploy the face detection use case. The ModelZoo enables you to train, evaluate, and automatically deploy any supported model. If you wish to use this project as part of the ModelZoo, please refer to the [Quickstart using stm32ai-modelzoo-services](#quickstart-using-stm32ai-modelzoo-services) section for instructions.
...
| :---- | :---- | -------------: |
| blazeface_front_128_quant_pc_ff_od_wider_face.tflite | STM32N6570-DK | 4 ms |
| blazeface_front_128_quant_pc_ff_od_wider_face.tflite | NUCLEO-N657X0-Q SPI | 4 ms |
| blazeface_front_128_quant_pc_ff_od_wider_face.tflite | NUCLEO-N657X0-Q UVCL | 7 ms |
...
To deploy your model using the ModelZoo, refer to the [Deployment README for STM32N6](https://github.com/STMicroelectronics/stm32ai-modelzoo-services/blob/main/face_detection/docs/README_DEPLOYMENT_STM32N6.md) for detailed instructions on deploying to either the STM32N6570-DK or the NUCLEO-N657X0-Q.

__Note__: This C-based application is referenced as a submodule of the ModelZoo repository at `application_code/face_detection`.
...
2. Program `Binary/STM32N6570-DK/STM32N6570-DK_GettingStarted_FaceDetection.hex`.
3. Set the board to [boot from flash mode](#boot-modes).
4. Power cycle the board.
5. Place a face in front of the camera to detect it.
...
1. Set the board to [development mode](#boot-modes).
2. Program `Binary/NUCLEO-N657X0-Q/USB-UVC-Display/NUCLEO-N657X0-Q_GettingStarted_FaceDetection-uvc.hex`.
3. Set the board to [boot from flash mode](#boot-modes).
...
7. Place a face in front of the camera to detect it.
...
1. Set the board to [development mode](#boot-modes).
2. Program `Binary/NUCLEO-N657X0-Q/SPI-Display/NUCLEO-N657X0-Q_GettingStarted_FaceDetection-spi.hex`.
3. Set the board to [boot from flash mode](#boot-modes).
4. Power cycle the board.
5. Place a face in front of the camera to detect it.
...
STM32_Programmer_CLI -c port=SWD mode=HOTPLUG -el $DKEL -hardRst -w Binary/STM32N6570-DK/STM32N6570-DK_GettingStarted_FaceDetection.hex
```
...
# USB/UVC
STM32_Programmer_CLI -c port=SWD mode=HOTPLUG -el $NUEL -hardRst -w Binary/NUCLEO-N657X0-Q/USB-UVC-Display/NUCLEO-N657X0-Q_GettingStarted_FaceDetection-uvc.hex

# SPI
STM32_Programmer_CLI -c port=SWD mode=HOTPLUG -el $NUEL -hardRst -w Binary/NUCLEO-N657X0-Q/SPI-Display/NUCLEO-N657X0-Q_GettingStarted_FaceDetection-spi.hex
```
